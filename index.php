...
if (isset($_POST['email']) && isset($_POST['password']) && isset($_POST['x'])) {
    ...
    if ($x === "user") {
        ...
        if ($user->userIsExist()) {
            $stm = $user->login();
            if ($stm) {
                $data = $stm->fetch(); // Fetch user data
                if ($data) {
                    session_start();
                    $_SESSION['id'] = $data['id'];
            ...
            echo '<div class="alert alert-danger"><strong>This user is not signed up.</strong> Please sign up and try again.</div>';
        }
    } else {
